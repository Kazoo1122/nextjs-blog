...
import { DATABASE_QUERY } from '../pages';
...
export async function getPosts(offset = 0) {
  ...
  const posts = (await getDbData(DATABASE_QUERY.ARTICLES, [offset, COUNT_PER_POSTS])) as any; //記事一覧をDBから取得
  const start = posts[0].id;
  const end = posts[COUNT_PER_POSTS - 1].id;
  const tags = (await getDbData(DATABASE_QUERY.TAGS_FOR_ARTICLES, [start, end])) as any; //タグと記事との紐付け一覧をDBから取得
...
*/
export async function getPostsDetail(id: number) {
  const { getDbData } = dbAPI();
