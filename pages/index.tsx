...
const Index = (props: { tags: TagProps[] }) => {
  ...
  const getKey = (pageIndex: number, previousPageData: PostProps[]) => {
    ...
    let url = `${process.env.BACKEND_URL}:${process.env.BACKEND_PORT}/api/posts-list?offset=${offset}&limit=${COUNT_PER_POSTS}`;
...
export const getServerSideProps: GetServerSideProps<{ tags: TagProps[] }> = async () => {
  const url = `${process.env.BACKEND_URL}:${process.env.BACKEND_PORT}/api/tags-list`;
